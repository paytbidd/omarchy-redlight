// Fade out: red -> gray -> color. `time` starts at 0 when this shader is applied.
...
void main() {
    ...
    vec3 lifted = mix(red, gray, min(1.0, a * 2.0));
    vec3 outc = mix(lifted, pix.rgb, max(0.0, a * 2.0 - 1.0));
